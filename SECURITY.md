...
- **The frontend bundle, on the web.** A compromised deployment could ship JavaScript that exfiltrates the phrase - the standard supply-chain risk of any web app; Subresource Integrity and reproducible web builds have not shipped yet. The signed native builds (macOS notarized, Windows Authenticode, Linux signed AppImage and deb, Android certificate-pinned APK) narrow that boundary: a fixed artifact cannot be changed under you between visits, so if your threat model includes us being coerced, prefer the native apps.
