...
**You can change your mind, in either direction.** Settings > Security > Your Phrase shows both modes side by side with what each one costs you, and a switch takes effect immediately. Leaving custodial mode deletes our stored copy, and is gated behind retyping three of your twelve words by position: the people most likely to click it are precisely the ones who never wrote the phrase down, because not needing to was the whole point of the mode. Returning to custodial mode uploads the phrase again, and is reachable only from an explicit choice you make in settings, never from a prompt, a nudge, or a recovery flow.

Earlier versions of this document called this a one-way ratchet, and until v0.271.0 leaving was the only direction that existed. Making it reversible carries a cost worth stating plainly: your phrase can now travel to our server from any signed-in device, rather than only in the seconds after signup. Both directions require a live provider session **and** an Ed25519 signature produced by your own account key, so a stolen session token alone can neither plant a phrase on your account nor remove one.

**Why offer it at all.** For a lot of people the realistic alternative to a custodial encrypted notes app is not a stricter one, it is a plaintext one, or a notes app owned by an advertising company. We would rather offer the trade honestly, default it to off, and let you move between the two modes knowingly, than pretend the trade does not exist. But if you came here for the guarantee the rest of this document describes, use the phrase flow, or switch to self-custody in settings.
