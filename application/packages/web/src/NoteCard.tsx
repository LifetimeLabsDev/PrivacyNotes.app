...
/**
 * Grid-mode tile: the parts NoteRow shows (type icon, title, preview, date,
 * flags, size, folder and tags), laid out as a card, with the short date as
 * the one deliberate difference. `tests/listItemParity.test.ts` renders both
 * and fails when a part shows in one and not the other. Shares NoteRowProps
 * so NotesList can render NoteRow or NoteCard interchangeably (same
 * handlers, same selection/context behavior).
 *
...
export default React.memo(function NoteCard({
  ...
  const faviconDomain = useMemo(() => (trashTint || locked ? '' : noteFaviconDomain(n)), [n, trashTint, locked]);
  ...
  return (
    ...
    >
      ...
      {n.type === 'link' && <span data-slot="ribbon" className="pn-ribbon" aria-hidden="true" />}
      {selectionMode && (
        <span
          onClick={(e) => {
            e.stopPropagation();
      ...
      <div className="pn-card-head flex items-center gap-2.5 min-w-0">
        <span data-slot="icon" className="pn-card-icbox contents">
      ...
      {(listPrefs.showDate || hasStatusIcons || sizeText) && (
        <div className="pn-card-meta flex items-center gap-1.5">
          {listPrefs.showDate && (
            <span data-slot="date" className="pn-card-date min-w-0 text-[12px] text-neutral-400 dark:text-neutral-600 whitespace-nowrap overflow-hidden text-ellipsis">
              {dateLabel} {dateValue}
            </span>
          )}
          <span data-slot="flags" className="pn-card-status shrink-0 flex items-center gap-1.5 text-accent">
            {pushFailure && (
