...
/// - only symbols from the package's OWN object member, or from `SwiftRs.o`,
///   this crate's runtime shim, which every consumer's package compiles as a
///   separate target and which nothing else promotes (archives embed copies
///   of dependency modules; promoting those in every archive duplicates
///   globals, and the shim's duplicates resolve from whichever archive the
///   linker reaches first: upstream pull request 82)
    ...
    for line in String::from_utf8_lossy(&nm.stdout).lines() {
        if line.ends_with(':') {
            ...
            if let Some(module) = member.strip_suffix(".o") {
                let norm_module = norm(module);
                in_own_member = norm_module == pkg || norm_module == runtime_shim;
                continue;
            }
        }
