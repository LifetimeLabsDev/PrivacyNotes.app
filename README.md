...
When you sign up with Google or Apple, you choose between self-custody and letting us store your phrase so that signing in on a new device is one click instead of twelve words. If you pick the second, your phrase sits on our servers encrypted under a key we hold. We do not read your notes, and nothing in the product looks at them: the server stores ciphertext, decryption happens on your device, and nothing in between indexes or analyses what you write. But **the capability is on our side**, and a threat model that counts intentions instead of capabilities is worthless, because intentions are not auditable and they survive neither a subpoena nor a change of ownership. So: a valid legal order, a deep enough compromise, or a dishonest future version of us could reach your plaintext. Self-custody users are exposed to none of that, because there is nothing on our side to compel.

You can change your mind either way, from Settings > Security > Your Phrase, and the change takes effect immediately. Leaving custodial mode deletes our copy, and asks you to retype three of your twelve words by position first, because the people most likely to click it are the ones who never wrote the phrase down. Going back uploads it again, and is reachable only from an explicit choice in settings. Both directions are signed with your own account key, so a stolen session token can neither plant a phrase nor delete one.

Earlier versions of this file called that a one-way ratchet. It was, until v0.271.0. Making it reversible means your phrase can now reach our server from any signed-in device rather than only at signup, which is a real widening and belongs here rather than in a changelog nobody reads.
